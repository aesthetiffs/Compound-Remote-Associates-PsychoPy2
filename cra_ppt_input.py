...
def cra_block(cra_trials, blockType):
    ...
    for this_cra in cra_handler:
        ...
        # get the solution(s) for this trial
        # there may be more than one solution for some problems, so it make sure that all solutions are considered
        # remove any spaces
        # add to this trial's list
        currentSolution = this_cra['solution']
        solution_list = currentSolution.split(',')
        ...
        while continueRoutine==True and (routineTimer.getTime() > 0):
            ...
            if len(cra_attempted) > 0:
                ...
                # Ask for the solution, then move onto the insight/analysis rating
                # Note: The screen will go away when input in a text box is required.
                # The input box only works when the screen is NOT in full screen.
                # Otherwise, you will not see the input box (it will hide behind the main experiment window).
                # To get around this, we HIDE the entire experiment window to reveal the input box.

                win.winHandle.set_visible(False)
                # creates the dictionary in which the solution data is stored
                CRAdict = {'Solution': ''}
                dictDlg = gui.DlgFromDict(dictionary=CRAdict,
                                          title='Enter your solution in the box below.')

                # Add the CRA + participant's solution, and make sure the answer is not empty before moving on (if accidental enter).
                # Move onto insight/analysis rating
                while len(CRAdict['Solution']) < 1:
                    # Give them one more attempt, and move on.
                    dictDlg = gui.DlgFromDict(dictionary=CRAdict,
                                              title='Enter your solution in the box below.')
                    # check for quit (the Esc key)
                    if endExpNow or event.getKeys(keyList=["escape"]):
                ...
                if dictDlg.OK:
                    # checks against the solution given by participant and adds correct/incorrect rating
                    # make sure to make it lowercase and remove any spaces, if applicable
                    # also remove any potential punctuation
                    cra_solution = CRAdict['Solution'].lower().replace(" ", "")
                    cra_solution = re.sub('([^\w]|[\d_])+', '', cra_solution)
                    cra_handler.addData('participant_solution', cra_solution)
                    if cra_solution in currentSolution:
                        cra_handler.addData('craCorrect', 1)
                    else:
                        cra_handler.addData('craCorrect', 0)

                # Once input is entered, asks how the participant solved the problem, by insight/analysis
...
if expInfo['which_part'] == '0':
    instruction('files/instructions/cra_instr_input.csv')
